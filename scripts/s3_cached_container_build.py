...
def docker_command_prefix() -> list[str]:
    configured = os.environ.get("DOCKER_COMMAND")
    if configured:
    ...
    for command in (["docker"], ["sudo", "docker"]):
        ...
        if result.returncode == 0:
            return command
    raise BuildFailure("docker is not usable via 'docker' or 'sudo docker'")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--android-ndk-version", default="27.2.12479018")
...
def main() -> int:
    ...
    docker_prefix = docker_command_prefix()
