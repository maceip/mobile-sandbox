...
NODE_ROOT="$WORK/node"
NODE_CMAKE_ROOT="$WORK/node-cmake-build"
...
prepare_worktrees() {
  rm -rf "$HOST_TOOLS" "$ANDROID_SRC" "$ANDROID_BUILD"
...
build_host_tools() {
  ...
  (
    ...
    find out -type f \( -name 'Makefile' -o -name '*.mk' \) -print0 | \
      xargs -0 sed -i \
        -e "s|$HOME_DIR/.local/bin/python3.14|$PYTHON|g" \
        -e "s|$UV_REAL_PYTHON_BIN|$PYTHON|g" \
        -e "s|$TOOLS_BIN/python3.14|$PYTHON|g"
    make -j"${HOST_JOBS:-32}" node node_mksnapshot genccode icupkg \
      >"$LOGS/node-host-build.log" 2>&1
  )
}
...
build_android() {
  ...
  (
    ...
    PYTHONPATH=tools/gyp/pylib:tools/v8_gypfiles NCG_TARGET_PLATFORM=android \
      "$NODE_PYTHON_BIN" tools/gyp_node.py -f "$NODE_CMAKE_ROOT/analyse.py" \
      >"$LOGS/node-android-analyse.log" 2>&1
    git ls-files "**.cmake" "**/CMakeLists.txt" | xargs -r rm -f
    PYTHONPATH=tools/gyp/pylib "$NODE_PYTHON_BIN" "$NODE_CMAKE_ROOT/generate.py" \
  ...
  cmake -S "$ANDROID_SRC" -B "$ANDROID_BUILD" -GNinja \
    ...
    -DNCG_HOST_TOOLS_DIR="$HOST_TOOLS/out/Release" \
...
package_outputs() {
  ...
  if [ -d "$WORK/node-host-tools/out/Release" ]; then
    tar -C "$WORK" -czf "$OUT/node-host-tools.tgz" node-host-tools/out/Release
  fi
